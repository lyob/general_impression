# SIL-comparison.py
'''
Comparing the representations in SIL and RIL -- single iteration
'''
...
class InputLayer(Layer):
    ...
    def forward(self):
        ...
        print('self.h, input layer', self.h) 
...
#define a feedforward Layer -- RIL
...
class FeedforwardLayerSIL(Layer):
    """defines a feedforward layer for the Helmholtz Machine"""
    def __init__(self, N, N_parent, N_child, nonlinearity, sigma_gen, sigma_rec, W_out = None, W_in = None, bias = False, top_layer = False):
        super().__init__(N, N_parent, N_child, nonlinearity, sigma_gen, sigma_rec)
        self.top_layer = top_layer
    ...
    def e_trace_reinforce(self):
        if not(self.bias is None):
            ...
            self.e_trace_update_list = [np.outer(self.nl.f_prime(self.h_pre_rec) * (self.h - self.h_mean_rec), self.child.h)]
        return self.e_trace_update_lis
...
class LayeredHMSIL():
    def __init__(self, N_vec, sigma_gen_vec, sigma_rec_vec):
        ...
        #construct the individual layers
        self.l0 = InputLayerSIL(N_vec[0], N_vec[1], nonlinearity, sigma_gen_vec[0], sigma_rec_vec[0])
        self.l1 = FeedforwardLayerSIL(N_vec[1], None, N_vec[0], nonlinearity, sigma_gen_vec[1], sigma_rec_vec[1], top_layer = True)
...
#Define simulation
class Simulation():
    def __init__(self, data, learn_alg, nn, nnSIL, train = True, compare_algs = [], epoch_num = 1, learning_stats = False, nn_record = False, phase_switch = False, starting_phase = 'wake'):
        self.data = data
    ...
    def run(self):
        ...
        latent = np.zeros((self.nnSIL.l1.N, T))
        loss = np.zeros((1, T))
        report_period_SIL = int(T*self.epoch_num/10)
        if report_period == 0
