...
class InputLayer(Layer):
    ...
    def __init__(self, N, N_parent, nonlinearity, sigma_gen, sigma_rec, W_out = None):
        ...
        print('the input layer has a sigma_gen value of', sigma_gen)
        print('the input layer has a sigma_rec value of', sigma_rec)
        #initialize W_out
    ...
    def forward_generative(self):
        # s_gen is dependent on the latent r from the previous time step:
        self.h_mean_gen = self.W_out @ self.parent.r_prev
    ...
    def forward_recognition(self, h_child):
        # s_rec has no dependence on other distributions 
    ...
    def forward(self):
        ...
        # calculate the complementary distribution 
        self.s_comp_prev = self.s_comp
        self.L_comp = self.Lambda - self.delta
        self.s_comp = self.L_comp * self.h_rec + (1-self.L_comp) * self.h_gen

        self.h_pred_gen = self.W_out @ self.parent.r_comp 
        # self.parent.r_comp is from the previous timestep -- this timestep's r_comp has not been calculated yet
        self.h_pred_rec = self.h_child

        self.layer_loss = 0.5 * ((np.sum(np.square(self.h - self.h_pred_gen)) / np.square(self.sigma_gen) - 
                                  np.sum(np.square(self.h - self.h_mean_rec)) / np.square(self.sigma_rec)) +
                                 (np.sum(np.square(self.h - self.h_pred_rec)) / np.square(self.sigma_rec) - 
                                  np.sum(np.square(self.h - self.h_mean_gen)) / np.square(self.sigma_gen)))
        # print('layer loss l0', self.layer_loss)
    ...
    def reset(self):
        ...
        self.s_prev = np.zeros((self.N,))
        self.s_comp = np.zeros((self.N,))
        self.s_comp_prev = np.zeros((self.N,))
...
class FeedforwardLayer(Layer):
    ...
    def __init__(self, N, N_parent, N_child, nonlinearity, sigma_gen, sigma_rec, W_out = None, W_in = None, bias = False, top_layer = False):
        ...
        print('the feedforward layer has a sigma_gen value of', sigma_gen)
        print('the feedforward layer has a sigma_rec value of', sigma_rec)
    ...
    def forward_generative(self):
        # r_gen is dependent on latent r from the prev timestep 
        self.h_mean_gen = self.transition_mat @ self.r_prev
        self.noise_gen = np.random.normal(scale = self.sigma_gen, size = (self.N,))
        self.h_gen = self.h_mean_gen + self.noise_gen        

    def forward_recognition(self):
        # r_rec is dependent on the stimulus distrib from the previous timestep 
        self.h_pre_rec = self.W_in @ self.child.s_prev + self.bias
    ...
    def forward(self):
        ...
        self.L_prime = self.Lambda + self.delta
        self.r_mean = ((self.L_prime * self.h_mean_rec * np.square(self.noise_gen) + (1-self.L_prime) * self.h_mean_gen * np.square(self.noise_rec)) /
                       (self.L_prime * np.square(self.noise_gen) + (1-self.L_prime) * np.square(self.noise_rec)))
        self.r_noise = self.noise_rec * self.noise_gen / (np.sqrt(self.L_prime * np.square(self.noise_rec)) + (1-self.L_prime) * np.square(self.noise_gen))
        self.r = self.r_mean + self.r_noise 
        self.h = self.r
        ...
        # calculate the complementary r distribution
        self.r_comp_prev = self.r_comp  # save the complementary r dist from the prev timestep 
        self.L_comp = self.Lambda - self.delta
        self.r_mean_comp = ((self.L_comp * self.h_mean_rec * np.square(self.noise_gen) + (1-self.L_comp) * self.h_mean_gen * np.square(self.noise_rec)) /
                            (self.L_comp * np.square(self.noise_gen) + (1-self.L_comp) * np.square(self.noise_rec)))
        self.r_noise_comp = self.noise_rec * self.noise_gen / (np.sqrt(self.L_prime * np.square(self.noise_rec)) + (1-self.L_prime) * np.square(self.noise_gen))
        self.r_comp = self.r_mean_comp + self.r_noise_comp 
        
        #Alternative calculation of r_comp
        # self.r_mean_comp = ((self.L_comp * self.h_mean_rec * np.square(self.sigma_gen) + (1-self.L_comp) * self.h_mean_gen * np.square(self.sigma_rec)) /
        #                     (self.L_comp * np.square(self.sigma_gen) + (1-self.L_comp) * np.square(self.sigma_rec)))
        # r_sigma_comp = self.sigma_gen * self.sigma_rec / np.sqrt(self.L_comp * np.square(self.sigma_gen) + (1-self.L_comp) * np.square(self.sigma_rec)))
        # self.r_noise_comp = np.random.normal(scale=r_sigma_comp, size=(self.N,))
        # self.r_comp = self.h_mean_comp + self.noise_h_comp
        ...
        self.h_pred_rec = self.nl.f(self.W_in @ self.child.s_comp_prev + self.bias)
        ...
        # print('layer loss l1', self.layer_loss)
    ...
    def reset(self):
        ...
        self.r_prev = np.zeros((self.N,))
        self.r_comp = np.zeros((self.N,))
        self.r_comp_prev = np.zeros((self.N,))
    ...
    def grad_rec(self):
        # we use the stimuli values from this timestep 
        a_hat = self.child.s
...
if __name__ == '__main__':
    ...
    if exp_params.mode in ('standard', 'MNIST', 'time_constant', 'switch_period', 'dimensionality', 'lr_optim', 'SNR', 'sinusoid'):
        ...
        # layer 0: sigma_gen: exp_params.sigma_obs_gen = 0.01, sigma_rec: exp_params.sigma_in = 0.01
        # layer 1: sigma_gen: sigma_latent_data = 0.5 * np.sqrt(0.1), sigma_rec: exp_params.sigma_latent = 0.01
