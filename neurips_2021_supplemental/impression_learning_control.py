...
class FeedforwardLayer(Layer):
    ...
    def __init__(self, N, N_parent, N_child, nonlinearity, sigma_gen, sigma_rec, W_out=None, W_in=None, bias=False,
                 top_layer=False):
...
class LayeredLearningAlgorithm():
    ...
    def assign_vars(self):
        ...
        for ii in range(0, len(self.nn.layer_list)):
            ...
            for jj in range(0, len(self.nn.layer_list[ii].params_list_rec)):
                self.nn.layer_list[ii].params_list_rec[jj] += self.learning_rate * self.update_list_rec[ii][
                    jj] / exp_params.recognition_scale
    ...
    def update_learning_stats(self):
        ...
        for ii in range(0, len(self.nn.layer_list)):
            for jj in range(0, len(self.update_list_rec[ii])):
                self.learning_stats['mean_update'][ii][jj] = (self.learning_stats['mean_update'][ii][jj] * self.N_prev +
                                                              self.update_list_rec[ii][jj]) / (self.N_prev + 1)
                self.learning_stats['moment_2'][ii][jj] = (self.learning_stats['moment_2'][ii][jj] * self.N_prev +
                                                           self.update_list_rec[ii][jj] ** 2) / (self.N_prev + 1)
        self.N_prev += 1
        return
    ...
    def get_learning_stats(self):
        ...
        for ii in range(0, len(self.nn.layer_list)):
            for jj in range(0, len(self.update_list_rec[ii])):
                self.variance[ii][jj] = self.learning_stats['moment_2'][ii][jj] - \
                                        self.learning_stats['mean_update'][ii][jj] ** 2
...
# Define simulation
class Simulation():
    def __init__(self, data, learn_alg, nn, train=True, compare_algs=[], epoch_num=1, learning_stats=False,
                 nn_record=False, phase_switch=False, starting_phase='wake'):
        self.data = data
...
if __name__ == '__main__':
    ...
    if exp_params.mode == 'Vocal_Digits':
        ...
        data_train, data_latent_train = simulate_data(n_latent, n_out, n_sample, mixing_matrix, transition_matrix,
                                                      sigma_latent=sigma_latent_data, sigma_out=0.01)
        data_test, data_latent_test = simulate_data(n_latent, n_out, n_test, mixing_matrix, transition_matrix,
                                                    sigma_latent=sigma_latent_data, sigma_out=0.01)
    ...
    if exp_params.mode in ('standard', 'MNIST', 'time_constant', 'switch_period', 'dimensionality', 'lr_optim', 'Vocal_Digits', 'sinusoid'):
        ...
        sim = Simulation(data_train, learn_alg, network, train=True, epoch_num=exp_params.epoch_num,
                         learning_stats=False, nn_record=True, starting_phase='wake')
        ...
        if (not (exp_params.mode == 'MNIST')):
            ...
            for ii in range(0, exp_params.gen_sim_num):
                gen_sim_temp = Simulation(data_test[:, 0:50], learn_alg, network, train=False,
    ...
    if exp_params.save:
        ...
        if exp_params.mode in (
        'standard', 'MNIST', 'time_constant', 'switch_period', 'dimensionality', 'Vocal_Digits', 'sinusoid'):
